...
static const char *const ATTR_NEED_PERIODIC_PINGS = "needs_pings";

static const String attrFormatted("valid_format"); // used as key and value
...
#else
  return false;
#endif // ifdef NV_CLEAR_BUTTON_CHECK
}
...
void wifiConfig_init(bool forceNvClear)
{
  ...
  if (forceNvClear || _shouldResetNv() || preferences.getString(attrFormatted.c_str()) != attrFormatted)
  {
#ifdef DEBUG
    delay(6666);
    ...
    preferences.putString(attrFormatted.c_str(), attrFormatted);
  ...
  char mqtt_server[_ATTR_MAX_LEN] = {0};
  preferences.getString(ATTR_MQTT_SERVER, mqtt_server, sizeof(mqtt_server));
  char mqtt_port[_ATTR_MAX_LEN] = DEFAULT_MQTT_PORT;
  preferences.getString(ATTR_MQTT_PORT, mqtt_port, sizeof(mqtt_port));
  char mqtt_username[_ATTR_MAX_LEN] = {0};
  preferences.getString(ATTR_MQTT_USERNAME, mqtt_username, sizeof(mqtt_username));
  char mqtt_password[_ATTR_MAX_LEN] = {0};
  preferences.getString(ATTR_MQTT_PASSWORD, mqtt_password, sizeof(mqtt_password));
  char mqtt_topic[_ATTR_MAX_LEN] = DEFAULT_MQTT_TOPIC;
  preferences.getString(ATTR_MQTT_TOPIC, mqtt_topic, sizeof(mqtt_topic));

  char needPeriodicPingsStr[_ATTR_MAX_LEN] = {0};
  if (preferences.getBool(ATTR_NEED_PERIODIC_PINGS))
    snprintf(needPeriodicPingsStr, _ATTR_MAX_LEN, "checked");
  ...
  {
#ifdef DEBUG
    Serial.printf("Starting wifiManager now\n");
#endif
#ifdef ESP_BOARD_LED
    digitalWrite(ESP_BOARD_LED, HIGH);
#endif
    ...
    String custom_need_pings_html = "placeholder=\"" + String(ATTR_NEED_PERIODIC_PINGS) + "\" type=\"checkbox\" ";
    custom_need_pings_html += needPeriodicPingsStr;
    WiFiManagerParameter custom_need_pings(ATTR_NEED_PERIODIC_PINGS, "needs periodic pings", ATTR_NEED_PERIODIC_PINGS,
                                           sizeof(needPeriodicPingsStr), custom_need_pings_html.c_str());
    ...
    wifiManager.addParameter(&custom_need_pings);
...
#endif
      if (strcmp(p.getID(), ATTR_NEED_PERIODIC_PINGS) == 0)
      {
        preferences.putBool(p.getID(), strlen(p.getValue()) > 0);
      }
      else
      {
  ...
  Serial.printf("%s: %s\n", ATTR_NEED_PERIODIC_PINGS, preferences.getBool(ATTR_NEED_PERIODIC_PINGS) ? "yes" : "no");
  ...
  _wifiConfigData.needPeriodicPings = preferences.getBool(ATTR_NEED_PERIODIC_PINGS);
