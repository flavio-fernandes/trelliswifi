...
TrellisCallback keyPressCallback(keyEvent evt)
{
  ...
  if (evt.bit.EDGE == SEESAW_KEYPAD_EDGE_RISING)
  {
    ...
    clearFlag(state.buttons.pressed, buttonIndex);
  ...
  static char buff[17];
  Serial.print("button ");
  snprintf(buff, sizeof(buff), "%02u", buttonIndex + 1);
  Serial.print(buff);
  if (evt.bit.EDGE == SEESAW_KEYPAD_EDGE_RISING)
    Serial.print(" DOWN");
  else
  ...
  snprintf(buff, sizeof(buff), "%016llx", state.buttons.pressed);
  Serial.print(buff);
  Serial.print(". delta: 0x");
  snprintf(buff, sizeof(buff), "%016llx", state.buttons.pressed ^ prevPressed);
  Serial.print(buff);
  Serial.println("");
#endif
  ...
  if (state.buttons.pressed)
  {
    for (int i = 0; i < 64; ++i)
    {
      if (getFlag(state.buttons.pressed, i))
        _bumpCounter(state.buttons.buttonPressedCounter[i]);
...
void initButtons(TickerScheduler &ts)
{
  ...
  ts.sched(buttons100msTick, 100);
  ts.sched(buttons1secTick, oneSec);
}
