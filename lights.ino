...
static void pressedButtonAnimation() {
  if (!state.buttons.pressed) return; // noop
...
#ifdef DEBUG
	Serial.print("Warning: Button "); Serial.print(i + 1, DEC); Serial.println(" is stuck!");
#endif
...
static uint64_t unpressedButtonUpdate() {
  if (!state.buttons.changedState) return 0; // noop
  ...
  for (int i=0; i<Y_DIM*X_DIM; ++i) {
	...
	! getFlag(state.buttons.abortedPendingPressEvent, i)) {
      ...
      static char buff[17];
      Serial.print("button ");
      snprintf(buff, sizeof(buff), "%02u", i + 1); Serial.print(buff);
      Serial.print(" released after ");
      snprintf(buff, sizeof(buff), "%3u", state.buttons.buttonPressedCounter[i]); Serial.print(buff);
...
void lightsFastTick() {
  ...
  if (state.buttons.pressed || state.buttons.changedState) {
...
#ifdef DEBUG
	    Serial.print(i + 1, DEC); Serial.print(" ");
#endif
	  }
	}
#ifdef DEBUG
	...
	for (int i=0; i < Y_DIM * X_DIM; ++i) state.buttons.buttonPressedCounter[i] = 0;
      }
    }
  }
}
...
void lights5secTick() {
  // if (isMqttConnected()) { trellis.setPixelColor(0, colorGreen); trellis.show(); }
}

void lights1minTick() {
  if (isBatteryLow()) startAnimationLowBattery();
}

void clearLights(bool callTrellisShow) {
  for (int i=0; i < Y_DIM * X_DIM; ++i) trellis.setPixelColor(i, 0);
  if (callTrellisShow) trellis.show();
}
...
static uint32_t applyBrightness(const void* /*LightUnit**/ lightUnitPtr,
  ...
  if (lightUnit.animation.pulse) {
    if (unitState.pulseGoingUp) {
      if (unitState.pulseBrightness >= (250 - brightnessIncr)) unitState.pulseGoingUp = !unitState.pulseGoingUp;
      else unitState.pulseBrightness += brightnessIncr;
    } else {
      if (unitState.pulseBrightness <= (2 + brightnessIncr)) unitState.pulseGoingUp = !unitState.pulseGoingUp;
      else unitState.pulseBrightness -= brightnessIncr;
    }
...
static void lightUnitIterate(const void* /*LightUnit**/ lightUnitPtr,
  ...
  if (isExpired && !lightUnit.animation.keepPixelWhenDone) color = 0;
  else if (lightUnit.animation.rainbowColor) color = Wheel();
  else if (lightUnit.animation.randomColor) color = 1;  // anything but zero is good here
  else if (lightUnit.animation.sameRandomColor) color = random(1, 0x00ffffff);

  if (lightUnit.animation.blink && ++unitState.tempCounter % 2 == 0) color = 0;
  else color = applyBrightness(lightUnitPtr, unitState, color);

  uint64_t pixels = lightUnit.pixelMask;
  if (lightUnit.animation.randomPixels) {
    // Last one includes all pixels
    if (isExpired && !lightUnit.animation.keepPixelWhenDone) pixels = ~0ULL;
    else {
      // Note: random() returns 31 bits. So, we will shift and xor it with previous value.
      pixels = random(0, 0x7fffffffUL); pixels ^= unitState.tempPixels * 2;
      unitState.tempPixels *= 0x100000000; unitState.tempPixels ^= pixels;
      // Override color to 0 on every 3rd frame
      if (! lightUnit.animation.blink) ++unitState.tempCounter;  // incr only if needed
      if (unitState.tempCounter % 3 == 0) color = 0;
    }
  }

  for (int i = 0; pixels && i < 64; ++i) {
    if (! clearFlag(pixels, i)) continue;
    if (color != 0 && lightUnit.animation.randomColor) {
      color = lightUnit.animation.rainbowColor ? Wheel() : random(1, 0x00ffffff);
    }
    if (pixelColorCache[i] == color) continue;
...
void lightUnitFinalIteration(void* /*LightUnit**/ lightUnitPtr,
  ...
  if (callTrellisShow &&
      origCacheVersion != pixelColorCacheVersion) trellis.show();
}
...
uint64_t getActivePixels() {
  ...
  for (int i = 0; i < 64; ++i) {
    ...
    if ((pixelColorCache[i] | cacheDirtyBit) != cacheDirtyBit) setFlag(result, i);
  }
  return result;
}
